...
const DEFAULT_TIMEOUT_MS = Number(process.env.HEXSTRIKE_TIMEOUT_MS || (DEFAULT_TIMEOUT_MIN * 60 * 1000));

function getBaseUrl() {
    const base = process.env.HEXSTRIKE_BASE_URL;
    if (!base) {
        throw new Error('HEXSTRIKE_BASE_URL is not configured');
    }
    ...
    try {
        ...
        if (text) {
            try {
                ...
                const parseError = new Error(`Failed to parse HexStrike response JSON: ${error.message}`);
        ...
        if (!response.ok) {
            const error = new Error(json?.error || `HexStrike request failed with status ${response.status}`);
...
function isConfigured() {
    return Boolean(process.env.HEXSTRIKE_BASE_URL);
}
